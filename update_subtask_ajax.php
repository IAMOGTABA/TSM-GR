...
// Debug logging
error_log("Subtask Update Debug - Received: subtask_id=$subtask_id, task_id=$task_id, status='$new_status'");

// Validate status
if (!in_array($new_status, ['to_do', 'done'])) {
    error_log("Subtask Update Debug - Invalid status: '$new_status'");
...
try {
    ...
    if ($total > 0 && $completed == $total) {
        ...
        $task_update = $pdo->prepare("UPDATE tasks SET status = 'completed' WHERE id = :task_id AND status != 'completed'");
        $result = $task_update->execute(['task_id' => $task_id]);
        if ($task_update->rowCount() > 0) {
            $task_status_changed = true;
            $new_task_status = 'completed';
        }
    } elseif ($new_status === 'to_do') {
