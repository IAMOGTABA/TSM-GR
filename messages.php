...
// Get tasks for dropdown based on user role (exclude completed tasks)
if ($role === 'admin') {
    // Admin can see all active tasks (not completed)
    $stmt = $pdo->prepare("
    ...
    // Employee can only see their assigned active tasks (not completed)
    $stmt = $pdo->prepare("
        SELECT t.id, t.title, t.status, t.priority, u.full_name as assigned_user 
        FROM tasks t
        LEFT JOIN users u ON t.assigned_to = u.id
