...
// Total Tasks
$stmt = $pdo->prepare("SELECT COUNT(*) as count FROM tasks WHERE (archived IS NULL OR archived = 0)");
$stmt->execute();
$stats['total'] = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

// Completed Tasks
$stmt = $pdo->prepare("SELECT COUNT(*) as count FROM tasks WHERE status = 'done' AND (archived IS NULL OR archived = 0)");
$stmt->execute();
$stats['completed'] = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

// Pending Tasks (pending + in_progress)
$stmt = $pdo->prepare("SELECT COUNT(*) as count FROM tasks WHERE status IN ('pending', 'in_progress') AND (archived IS NULL OR archived = 0)");
$stmt->execute();
$stats['pending'] = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

// Overdue Tasks
$today = date('Y-m-d');
$stmt = $pdo->prepare("SELECT COUNT(*) as count FROM tasks WHERE due_date < :today AND status != 'done' AND (archived IS NULL OR archived = 0)");
$stmt->execute(['today' => $today]);
$stats['overdue'] = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

// Get recent tasks with proper deadline display (excluding archived tasks)
$stmt = $pdo->prepare("
    SELECT t.*, u.full_name AS assigned_user,
           CASE 
               WHEN t.due_date IS NULL THEN 'No deadline'
               WHEN t.due_date < CURDATE() AND t.status != 'done' THEN CONCAT('Overdue: ', DATE_FORMAT(t.due_date, '%M %d, %Y'))
               WHEN t.due_date = CURDATE() THEN 'Due today'
               ELSE DATE_FORMAT(t.due_date, '%M %d, %Y')
...
if (empty($recent_activities)) {
    ...
    $fallback_stmt = $pdo->prepare("
        SELECT t.id as task_id, t.title as task_title, u.full_name as user_name, t.updated_at
        FROM tasks t 
        JOIN users u ON t.assigned_to = u.id
        WHERE t.status = 'done' AND DATE(t.updated_at) = CURDATE() AND (t.archived IS NULL OR t.archived = 0)
    ...
    try {
        ...
        foreach ($fallback_tasks as $task) {
            $recent_activities[] = [
                ...
                'time' => date('g:i A', strtotime($task['updated_at'])),
...
for ($i = 6; $i >= 0; $i--) {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM tasks WHERE DATE(updated_at) = :date AND status = 'done'");
...
$employee_performance = $pdo->prepare("
    ...
    FROM users u
    LEFT JOIN tasks t ON u.id = t.assigned_to AND (t.archived IS NULL OR t.archived = 0)
...
foreach ($employee_stats as $employee) {
    $completed_tasks_stmt = $pdo->prepare("
        SELECT t.id, t.title, t.description, t.completion_date, t.created_at,
               DATEDIFF(t.completion_date, t.created_at) as days_to_complete
        FROM tasks t
        WHERE t.assigned_to = :employee_id AND t.status = 'done'
                ...
                <div class="card-body">
                    ...
                    <div class="task-cards-container">
                        <?php foreach ($recent_tasks as $task): 
                            $is_overdue = !empty($task['due_date']) && strtotime($task['due_date']) < strtotime('today') && $task['status'] != 'done';
                        ?>
                        <div class="task-card priority-<?php echo $task['priority']; ?>">
                            <div class="task-card-body">
                                ...
                                <div class="task-card-info">
                                    ...
                                    <span class="<?php echo (strpos($task['deadline_display'], 'Overdue') !== false) ? 'overdue' : ''; ?>">
                                        <?php echo htmlspecialchars($task['deadline_display']); ?>
                                    </span>
                                </div>
                            ...
                            <div class="task-card-footer">
                                <?php if ($task['status'] === 'done'): ?>
                                    <span class="status-badge status-<?php echo $task['status']; ?> clickable-done" 
                            ...
                            <?php else: ?>
                                <div class="performance-table">
                                    <div class="table-header">
                                        ...
                                        <div style="text-align: center;"><i class="fas fa-check"></i> Completed</div>
                                    ...
                                    <div class="table-row">
                                        ...
                                        <div style="text-align: center;" data-label="Completed:">
